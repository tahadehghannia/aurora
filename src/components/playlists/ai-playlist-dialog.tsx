...
const EXAMPLES = ["music for studying at night", "driving alone at night", "something like my favorite songs but calmer"];
...
export function AiPlaylistDialog() {
  ...
  const generate = async () => {
    ...
    setGenerating(true);
    try {
      const playlist = await fetchJson<GeneratedPlaylist>("/api/ai/playlist", {
        method: "POST",
        body: JSON.stringify({ prompt: prompt.trim() }),
      });
      setResult(playlist);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Couldn't generate a playlist for that.");
    } finally {
      setGenerating(false);
    }
  };
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await fetchJson("/api/ai/playlist/save", {
        method: "POST",
        body: JSON.stringify({
          title: result.title,
          description: result.description,
          songIds: result.items.map((i) => i.card.id),
        }),
      });
      ...
      toast.error(err instanceof Error ? err.message : "Couldn't save that playlist.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>AI Playlist</DialogTitle>
        </DialogHeader>

        {!result ? (
          <div className="space-y-3 py-2">
            <Textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder={EXAMPLES[0]} rows={3} autoFocus />
